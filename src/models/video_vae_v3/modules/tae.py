...
TWorkItem = namedtuple("TWorkItem", ["xt", "i"])
        ...
        if checkpoint_path is not None:
            ...
            if ext == ".pth":
                ...
                if load_file is None:
                     raise ValueError("safetensors library not installed")
                state_dict = load_file(checkpoint_path, device="cpu")
            else:
                # Handle case where checkpoint_path might not have extension or is just a name
                # Default to torch.load if valid file
                if os.path.isfile(checkpoint_path):
                     state_dict = torch.load(checkpoint_path, map_location="cpu", weights_only=True)
                else:
                    # If file doesn't exist, we might be initializing structure only
                    # But if path was passed, expected to load.
                    # Just print warning if debug
    ...
    def patch_tgrow_layers(self, sd):
        ...
        for i, layer in enumerate(self.decoder):
            if isinstance(layer, TGrow):
                ...
                if key in sd and key in new_sd:
